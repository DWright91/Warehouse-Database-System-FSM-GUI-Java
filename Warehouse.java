...
public class Warehouse implements Serializable {
    ...
    public List<Client> getClientsWithNoTransactionsInLastSixMonths() {
        List<Client> clientsWithNoTransactions = new ArrayList<>();
        LocalDateTime sixMonthsAgo = LocalDateTime.now().minusMonths(6); // Calculate the date six months ago

        // Get the Iterator for clients
        Iterator<Client> clientIterator = clientList.getClients();

        while (clientIterator.hasNext()) {
            Client client = clientIterator.next();

            // Check if the client has any invoices in the last six months
            boolean hasTransactionsInLastSixMonths = hasTransactionsInLastSixMonths(client, sixMonthsAgo);

            // If the client has no transactions in the last six months, add them to the result list
            if (!hasTransactionsInLastSixMonths) {
                clientsWithNoTransactions.add(client);
            }
        }

        return clientsWithNoTransactions;
    }

    private boolean hasTransactionsInLastSixMonths(Client client, LocalDateTime sixMonthsAgo) {
        // Iterate through the client's invoices
        for (Invoice invoice : invoiceList.getInvoicesForClient(client.getClientId())) {
            // Check if the invoice date is within the last six months
            if (invoice.getInvoiceDate().isAfter(sixMonthsAgo)) {
                return true; // Client has had transactions in the last six months
            }
        }

        // If no transactions were found in the last six months, return false
        return false;
    }
    ...
    public List<Client> getClientsWithOutstandingBalance() {
        List<Client> clientsWithOutstandingBalance = new ArrayList<>();
        Iterator<Client> clientIterator = clientList.getClients();

        while (clientIterator.hasNext()) {
            Client client = clientIterator.next();
            if (client.getBalance() > 0) {
                clientsWithOutstandingBalance.add(client);
            }
        }

        return clientsWithOutstandingBalance;
    }
